...
def follow():
    ...
    try:
        ...
        return redirect(url_for("home"))
    user_info = db.users.find_one({"id": payload["id"]})
    target_id_receive = request.form["target_id_give"]
    target_pic_receive = request.form["target_pic_give"]
    doc = {
        "id": user_info["id"],
        "target_id": target_id_receive,
        "target_pic": target_pic_receive
    }
    db.follow.insert_one(doc)
    return jsonify({"result": "success", 'msg': '팔로우 성공'})
@app.route("/follow", methods=["GET"])
def following_get():
    following_list = list(db.follow.find({}, {'_id': False}))
    return jsonify({'following': following_list})
@app.route('/comment', methods=['POST'])
def comment_post():
    comment_txt_receive = request.form["comment_txt_give"]
    doc = {
        'comment_txt': comment_txt_receive,
    }
    db.content.insert_one(doc)
    return jsonify({'msg': '댓글 작성!'})
@app.route('/comment', methods=['GET'])
def comment_get():
    comments = list(db.content.find({}, {'_id': False}))
